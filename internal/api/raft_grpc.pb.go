...
const (
	RaftService_RequestVote_FullMethodName    = "/rafty.RaftService/RequestVote"
	RaftService_AppendEntries_FullMethodName  = "/rafty.RaftService/AppendEntries"
	RaftService_ProposeCommand_FullMethodName = "/rafty.RaftService/ProposeCommand"
)
...
type RaftServiceClient interface {
	...
	// used to propose a command to the leader from a follower
	ProposeCommand(ctx context.Context, in *ProposeRequest, opts ...grpc.CallOption) (*AppendEntriesResponse, error)
}
...
func (c *raftServiceClient) ProposeCommand(ctx context.Context, in *ProposeRequest, opts ...grpc.CallOption) (*AppendEntriesResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AppendEntriesResponse)
	err := c.cc.Invoke(ctx, RaftService_ProposeCommand_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type RaftServiceServer interface {
	...
	// used to propose a command to the leader from a follower
	ProposeCommand(context.Context, *ProposeRequest) (*AppendEntriesResponse, error)
}
...
func (UnimplementedRaftServiceServer) ProposeCommand(context.Context, *ProposeRequest) (*AppendEntriesResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method ProposeCommand not implemented")
}
...
func _RaftService_ProposeCommand_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ProposeRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(RaftServiceServer).ProposeCommand(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: RaftService_ProposeCommand_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RaftServiceServer).ProposeCommand(ctx, req.(*ProposeRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var RaftService_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "ProposeCommand",
			Handler:    _RaftService_ProposeCommand_Handler,
		},
	},
